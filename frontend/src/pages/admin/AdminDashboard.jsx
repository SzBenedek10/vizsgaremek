...
export default function AdminDashboard() {
  ...
  const [openUzenetDialog, setOpenUzenetDialog] = useState(false); 
 ...
 // --- KÉP KIVÁLASZTÁSA (Csak elmentjük a fájlt az állapotba) ---
  ...
  const handleSaveBor = async () => {
    try {
      ...
      formData.append('ar', borForm.ar);
      formData.append('keszlet', borForm.keszlet);
      formData.append('leiras', borForm.leiras);
      ...
      if (editingBor) {
        await axios.put(`http://localhost:5000/api/borok/${editingBor.id}`, formData, config);
      } else {
        await axios.post('http://localhost:5000/api/borok', formData, config);
      }
      
      Swal.fire('Siker', 'Bor mentve!', 'success'); 
  ...
  const handleOpenBorDialog = (bor = null) => {
    ...
    else { setEditingBor(null); setBorForm({ nev: '', evjarat: '2023', ar: '', keszlet: '0', leiras: '', bor_szin_id: 1, alkoholfok: '', kep: null }); }
  ...
  const handleSaveUzenet = async () => {
    try {
      ...
      Swal.fire('Siker', 'Üzenet módosítva!', 'success'); setOpenUzenetDialog(false); fetchUzenetek();
      ...
      {tabIndex === 0 && (
        <>
          ...
          <TableContainer component={Paper}>
            <Table>
              <TableHead sx={{ bgcolor: '#eee' }}><TableRow><TableCell>Kép</TableCell><TableCell>Név</TableCell><TableCell>Évjárat</TableCell><TableCell>Ár</TableCell><TableCell>Készlet</TableCell><TableCell align="center">Műveletek</TableCell></TableRow></TableHead>
              <TableBody>{borok.map((bor) => (<TableRow key={bor.id}>
                <TableCell>
                  {bor.kep ? <img src={bor.kep} alt={bor.nev} style={{ width: 40, height: 40, objectFit: 'cover', borderRadius: '4px' }}/> : "Nincs kép"}
                </TableCell>
                <TableCell>{bor.nev}</TableCell><TableCell>{bor.evjarat}</TableCell><TableCell>{bor.ar} Ft</TableCell><TableCell>{bor.keszlet} db</TableCell>
                <TableCell align="center"><Button color="primary" onClick={() => handleOpenBorDialog(bor)}><EditIcon /></Button><Button color="error" onClick={() => handleDeleteBor(bor.id)}><DeleteIcon /></Button></TableCell>
              </TableRow>))}</TableBody>
            </Table>
          </TableContainer>
        </>
      ...
      <Dialog open={openBorDialog} onClose={() => setOpenBorDialog(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          ...
          <FormControl fullWidth margin="dense">
            ...
            <Select
              value={borForm.bor_szin_id || ''}
              label="Bor Színe"
              onChange={(e) => setBorForm({...borForm, bor_szin_id: e.target.value})}
            >
              {borSzinek.map((szin) => (
                <MenuItem key={szin.id} value={szin.id}>{szin.nev}</MenuItem>
              ))}
            </Select>
          </FormControl>
          ...
          <Box sx={{ mt: 3, p: 2, border: '1px dashed #ccc', borderRadius: 2, textAlign: 'center' }}>
           ...
           {(borForm.kep_elonezet || borForm.kep) && (
              <Box sx={{ mt: 1 }}>
                <Typography variant="caption" display="block" color="textSecondary" mb={1}>Kép előnézete:</Typography>
                <img 
                  src={borForm.kep_elonezet ? borForm.kep_elonezet : `http://localhost:5000${borForm.kep}`} 
                  alt="Előnézet" 
